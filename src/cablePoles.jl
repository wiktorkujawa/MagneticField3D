global function ShowThreePhaseRoute(p_open::Ref{Bool})
          ...
          @cstatic startLineLeft=Cfloat[0.10, 0.20, 0.30] startLineCenter=Cfloat[0.10, 0.20, 0.30] startLineRight=Cfloat[0.10, 0.20, 0.30] endLineLeft=Cfloat[0.10, 0.20, 0.30] endLineCenter=Cfloat[0.10, 0.20, 0.30] endLineRight=Cfloat[0.10, 0.20, 0.30]  gapL2=Cfloat[0.10, 0.20, 0.30] gapL3=Cfloat[0.10, 0.20, 0.30] formationType=Cint(1) I=Cfloat(640.0) Angle=Cfloat(0.0) phaseAngle=Cfloat(0.0) lineLength=Cfloat(500.0)  inputMethod=Cint(1) xpole=Cfloat(0.0) ypole=Cfloat(0.0) zstart=Cfloat(0.0) zend=Cfloat(10.0) gapSecond=Cfloat[0.0,0.0,0.0] gapThird=Cfloat[0.0,0.0,0.0] phaseOrder=Cint(0) phaseMultiplier=Cint[0,-1,1] startmidLine=Cfloat[0.10, 0.20, 0.30] gapFlat=Cfloat(4.0) gapTrefoil=Cfloat(4.0) begin
            ...
            if inputMethod==0
              ...
              if CImGui.Button("Add route")
                push!(S,[startLineLeft;endLineLeft])
                push!(Itab, I*exp(im*(phaseMultiplier[1]*phaseShift+Angle)))
                push!(S,[startLineLeft+gapL2;endLineLeft+gapL2])
                push!(Itab, I*exp(im*(phaseMultiplier[2]*phaseShift+Angle)))
                push!(S,[startLineLeft+gapL3;endLineLeft+gapL3])
                push!(Itab, I*exp(im*(phaseMultiplier[3]*phaseShift+Angle)))
                bufLength=sqrt(sum((startLineLeft-endLineLeft).^2))
                push!(Length,bufLength)
                push!(Length,bufLength)
                push!(Length,bufLength)
              end
            else
              ...
              if CImGui.Button("Add route")
                ...
                push!(S,[startLineLeft;endLineLeft])
                push!(S,[startLineCenter;endLineCenter])
                push!(S,[startLineRight;endLineRight])
                # bufLength=sqrt(lineLength*lineLength+(zstart-zend)^2)    
                push!(Itab, I*exp(im*(phaseMultiplier[1]*phaseShift+phaseAngle)))
                push!(Itab, I*exp(im*(phaseMultiplier[2]*phaseShift+phaseAngle)))
                push!(Itab, I*exp(im*(phaseMultiplier[3]*phaseShift+phaseAngle)))

                push!(Length,lineLength)
                push!(Length,lineLength)
                push!(Length,lineLength)
              end
            end
